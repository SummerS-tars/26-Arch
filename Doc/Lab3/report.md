...
### 2.4 在功能通过后，再补上板路径需要的适配

Lab3 和前两次实验不同的一点是，这次还要求上板测试。  
因此在仿真通过之后，还需要检查板级路径是否有 Vivado 不兼容或不稳定的写法。

这部分不是再次改 CPU 指令语义，而是确保：

- 低地址 MMIO 访问不会把 Difftest 判断搞乱
- 板级 `device.sv` 里的寄存器初始化方式对 Vivado 友好
- 板级外设 ready 行为不会无谓阻塞 CPU
- 顶层未使用输入不要悬空
...
这次 Lab3 涉及的文件比前两次更分散，但总体仍然沿用“顶层负责时序，子模块负责组合语义”的结构。
...
### 4.4 `load-use hazard` 和 `mem_wait` 叠在一起时，会出现“load 被吃掉”的隐蔽 bug
...
### 4.6 上板路径还有一层独立问题：Vivado 兼容性

Lab3 通过仿真之后，还额外检查了板级路径。  
这一部分的问题和 Lab3 指令语义不同，主要是：

- `device.sv` 中原本存在声明时赋初值的写法
- 板级 `ready` 行为过宽
- `VTop` 的中断输入原本悬空

这些问题在 Verilator 下不一定会立刻暴露，但在 Vivado 上更容易变成不稳定因素。  
因此在上板前又做了一轮小范围适配，让板级路径更稳。
...
Lab3 和前两次实验不同，这次还要求上板。  
因此报告中除了仿真结果外，还需要说明上板适配情况。
